...
class TabularLearningAlg:
    ...
    def iterate(self):
        ...
        while (1):
            ...
            fellOfCliff = hasFallenCliff(currState)
            if fellOfCliff:
                nextState = self.initialState

            # Choose A' from S' using policy derived from Q
            nextAction = self.behaviorPolicy.selectNewAction(self.qMatrix, nextState,
                                                             possibleNextCliffActions(nextState))
            if (self.behaviorPolicy == self.targetPolicy):
                qAction = nextAction
            else:
                qAction = self.targetPolicy.selectNewAction(self.qMatrix, nextState,
                                                            possibleNextCliffActions(nextState))
            ...
            self.qMatrix[currState, currAction] = currentQValue + self.stepSize * (
...
def displayPath(ax, avgQs, initialState, terminalState, gridX, gridY, arrowColor, arrowOffsetX, arrowOffsetY,
                arrowSize):
    ...
    while (currState != terminalState):
        ...
        ax.arrow(currStatePos[0] - directionalArrowOffset[0], currStatePos[1] - directionalArrowOffset[1],
                 orientation[0] * arrowSize, orientation[1] * arrowSize, head_width=0.05, head_length=0.1,
                 ec=arrowColor, color=arrowColor)
...
def main():
    ...
    for r in range(numberOfRuns):
        sarsa = TabularLearningAlg(numberOfStates, numberOfActionsPerState, initialState, terminalState, discountFactor,
                                   stepSize, rewardMatrix, eGreedy, eGreedy)
        qLearning = TabularLearningAlg(numberOfStates, numberOfActionsPerState, initialState, terminalState,
                                       discountFactor, stepSize, rewardMatrix, greedy, eGreedy)
