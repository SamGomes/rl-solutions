...
class LinearApproximationLearningAlg:
	def __init__(self, numberOfStates, numberOfActionsPerState, initialState, \
		...
		self.currAction = self.behaviorPolicy.selectNewAction(self.currState, self.possibleActions, self.actionsFeatures, self.wVector)

	def iterate(self):
			nextState = getNextState(self.currState, self.currAction)
			currReward = self.rewardMatrix[self.currState, self.currAction]

			nextAction = self.behaviorPolicy.selectNewAction(nextState, self.possibleActions, self.actionsFeatures, self.wVector)
			if(self.behaviorPolicy==self.targetPolicy):
				qAction = nextAction
			else:
				qAction = self.targetPolicy.selectNewAction(nextState, self.possibleActions, self.actionsFeatures, self.wVector)
			...
			self.wVector = self.wVector + self.stepSize * self.actionsFeatures[self.currAction][self.currState]*(currReward + self.discountFactor * futureQValue - currentQValue)
...
def main():
	...
	for r in range(numberOfRuns):
		qLearning = LinearApproximationLearningAlg(numberOfStates, numberOfActionsPerState, 0, discountFactor, stepSize, rewardMatrix, greedy, epsilonAB, actionsFeatures, initialWVector)
		sarsa = LinearApproximationLearningAlg(numberOfStates, numberOfActionsPerState, 0, discountFactor, stepSize, rewardMatrix, epsilonAB, epsilonAB, actionsFeatures, initialWVector)
